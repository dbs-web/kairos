...
export default function SocialMediaSubTab({ Icon, text, href, active = false }: SocialMediaSubTabProps) {
    return (
        <li
            className={`${
                active
                    ? 'border-primary bg-gradient-to-r from-primary/10 to-primary/5 text-primary shadow-sm border'
                    : 'text-foreground/60 hover:bg-muted/30 hover:text-foreground/80'
            } flex flex-nowrap items-center justify-center gap-x-2 text-nowrap rounded-md p-1.5 px-3 font-medium text-sm transition-all duration-200`}
        >
            {Icon && (
